...
import CardInformation from "../information/card-information";
import CardMate from "../information/card-mate";
import CardChild from "../information/card-child";
...
const PageAddEditPersonnel = (): JSX.Element => {
  ...
  const { mutate: actionSave, isPending: isLoadingActionSave } = useMutation({
    mutationFn: async (payload: MasterUserRelationModel) => {
      if (action === "create") {
        return await _MasterUserApi().createRelation(payload);
      } else if (action === "edit") {
        // const {createdDate, updatedDate, deletedDate, id, no,...newData} = payload
...
export default PageAddEditPersonnel;
